								...
								timetableString += 	'<div class="timetable-item">'
														...
														+ '<div id="' + el.attr('id') + '-popup-' + timetables[t].id + '" class="timetable-popup zoom-anim-dialog mfp-hide">'
															...
															+ '<div class="popup-body">'
																...
																+ '<div class="timetable-desc">' + '<p class="timetable-desc">' + timetables[t].description + '</p> </div>'
																+ '<button class="timetable-popup-button change-color-button" onclick="toggleDropdown()"><i class="fas fa-palette"></i> Farbe ändern</button>'
																+ '<div class="dropdown">'
																	+ '<div id="myDropdown" class="dropdown-content">'
																		+ '<button class="dropdown-selector dd-button-1" id="' + timetables[t].name + '" onclick="changeEntryColor(this, 1)">Türkis</button>'
																		+ '<button class="dropdown-selector dd-button-2" id="' + timetables[t].name + '" onclick="changeEntryColor(this, 2)">Blau</button>'
																		+ '<button class="dropdown-selector dd-button-3" id="' + timetables[t].name + '" onclick="changeEntryColor(this, 3)">Gelb</button>'
																		+ '<button class="dropdown-selector dd-button-4" id="' + timetables[t].name + '" onclick="changeEntryColor(this, 4)">Orange</button>'
																	+ '</div>'
																+ '</div>'
																+ '<button class="timetable-popup-button delete-button" id="' + timetables[t].name + '" onclick="deleteEntry(this)"><i class="fas fa-trash-alt"></i> Eintrag löschen</button>'
															+ '</div>'
														+ '</div>'
													+ '</div>';
							}
...
jQuery(document).ready(function(){
	jQuery('.tiva-timetable').each(function(index) {
		...
		if (source == 'json') { // Get timetables from json file
			if (mode == 'day') {
				var timetable_json = timetableMaster;
			} else {
				timetable_json = timetableMaster;
			}
				...
				for (var i = 0; i < timetable_json.items.length; i++) {
					tiva_timetables.push(timetable_json.items[i]);
				}
